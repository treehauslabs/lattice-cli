...
struct QueryCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "query",
        abstract: "Query chain state"
    )

    @Argument(help: "Query: height, tip, blocks, spec, or balance <address>")
    ...
    func run() async throws {
        guard !expression.isEmpty else {
            ...
            print("")
            print("  Usage:")
            print("    \(Style.dim)lattice query height\(Style.reset)")
            print("    \(Style.dim)lattice query tip\(Style.reset)")
            print("    \(Style.dim)lattice query blocks\(Style.reset)")
        ...
        case "balance":
            guard expression.count > 1 else {
                printError("Usage: lattice query balance <address>")
                throw ExitCode.failure
            }
            printWarning("Balance queries require a running node with resolved state.")
            printWarning("Use the JSON-RPC gateway: lattice_getBalance via HTTP")
